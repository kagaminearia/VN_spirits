label xHE:
    scene bg_black with dissolve
    show movie_side
    show bg_sunset1 behind movie_side with dissolve
    mei_speaking "……嗯？好。"
    stop music fadeout 0.5
    mei "虽然没想到她开口是说这个，但我毫不客气地点头，接过她递来的外套披在身上。"
    mei "厚重的布料还带着余温，压住我的肩膀，隔绝了转凉的空气。"
    mei "外套换到梅雨身上后，向夏仍然攥着拳，久久没有改变动作，也没有再说话。"
    $ renpy.music.play(music.xiang_hug, channel="music", loop=True, fadein=0.5)
    mei_speaking "……"
    mei_speaking "你到这里的，呃，要做的事，做完了吗？"
    xiang_speaking "啊……没，不，也算是吧，做完了。"
    mei "向夏还有些恍神，听到声音后脸上才匆忙扬起平日的笑容，我却只觉得有些难看。"
    mei_speaking "你这么相信所谓的传说，就是为了祝灵节吗？"
    xiang_speaking "嗯，因为我……我没文化？封建迷信？"
    mei_speaking "……"
    xiang_speaking "哎呀，别那么看我，开玩笑的。不是说过嘛，听说祝灵节的神秘，能够看到最想看到的人。"
    mei_speaking "嗯？"
    xiang_speaking "好吧，我是在做梦，我信了……"
    xiang_speaking "我只是在想能不能再见到我妈。"
    mei_speaking "……啊？"
    xiang_speaking "在想……想问她，都已经放弃过我这么多次，何必还要在最后帮我那一次。"
    mei_speaking "……"
    xiang_speaking "搞得我……"
    mei_speaking "……{p}啧……小可怜。"
    mei "我实在不擅长面对这种话题，只好挑了挑眉，含糊着叹息过去。"
    mei "于是又冷场了。"
    mei "好在……她似乎已经在短时间内说服好自己，还用拳头用力挥了挥空气，一副打气的样子。"
    xiang_speaking "算了！不说那些……反正本来就是不可能的事情，是我做梦。"
    mei "她左手摸过被长袖盖住的右手臂，似是有些恍神，但最终还是又笑出来。"
    show bg_white with dissolve
    pause(0.5)
    hide bg_white with dissolve
    mei_speaking "……怎么样？你……"
    xiang_speaking "我都说，无所谓咯。"
    mei_speaking "哦……那好。抱歉啊，我没话找话，提到这个话题。"
    xiang_speaking "没有啊，之前不是也说过嘛。"
    mei "我抿了抿嘴，看到她似乎真的没事之后，在心里悄悄松了口气。"
    mei "事实上，虽然也有愧疚，但我好像并没有后悔提到这些。"
    mei "也许是因为……隐约中，她的所有事情，都让我感到好奇。"
    mei_speaking "所以……"
    xiang_speaking "啊？"
    mei_speaking "在我没话找话之前，你一直盯着我，到底要说什么？"
    xiang_speaking "……"
    xiang_speaking "就是，你……"
    xiang_speaking "……"
    mei "向夏翕动着嘴唇，张开嘴，又闭上，还是没憋出一句完整的话。"
    stop music fadeout 0.5
    mei_speaking "……"

    hide movie_side with dissolve
    $ renpy.music.play(music.xiang_love_you, channel="music", loop=True, fadein=0.5)
    mei_speaking "你不是讨厌我吗，怎么还拿衣服过来，还跟我聊这么久。"
    xiang_speaking "……"
    xiang_speaking "这个嘛……要是，你生病了，多不好啊。"
    mei_speaking "那也是我的事情吧，我——"
    xiang_speaking "我是担心你。"
    mei "话没说完，向夏马上着急地搭腔。"
    mei "嗯……我并没有回答，只是轻轻摸了摸黑色的外套，感受上面的温度。"
    xiang_speaking "就，就是这样。"
    mei_speaking "所以说……"
    mei "我看她一眼，又低下头，不由得捏紧了手指。"
    mei_speaking "你不是讨厌我吗，有什么好担心的。"
    xiang_speaking "……"
    xiang_speaking "你真的，啧，倔强。"
    mei_speaking "……谢谢你的夸奖。"
    mei "向夏长长——长长地叹了一口气，听声音，却又是像在笑。"
    mei "……这人到底什么意思？"
    xiang_speaking "我是很讨厌你。"
    mei_speaking "……哦。"
    mei "所以要特意过来再强调一遍？至于吗……"
    xiang_speaking "你知道的，我很笨，容易被骗。以前吃过很多亏，所以我一直觉得，以后都不要和任何人扯上关系就好了。不扯上关系，就不会有麻烦。"
    xiang_speaking "可是你不一样。我总是特别在乎你，不管在什么时候，总是会想到你。有的时候比起自己，竟然先考虑到你的想法。你开心，我就开心，你不高兴，我也不高兴了。"
    xiang_speaking "我明明已经下定决心了，结果就因为你。那天你走之后，我晚上都没睡着，心脏还痛，我都怀疑你是不是把心脏病传染给我了。"
    xiang_speaking "就连刚才……刚才也是。我明知道有话直说会减少很多麻烦，我一直相信的。但是……"
    xiang_speaking "面对你，我都不知道该如何开口。"
    xiang_speaking "这种情况也不是第一次了！跟你一起的时候，总是这样……总是影响到我，和其他人都不同。"
    xiang_speaking "难道我不该讨厌你吗？真是的……"
    mei_speaking "嘶……"
    mei "向夏一连串的连珠炮很快，惊得我什么也来不及想，下意识地移开了脸，以免暴露自己的表情。"
    mei_speaking "你……"
    mei "我万万没想到她竟然会说出这么一番话。"
    mei "以至于，我下意识发出的声音都有些颤抖。"
    mei "我连忙停住，先轻轻喘了口气，这才转回头看她。这人，真是……每次都能说出最震撼我的话。"
    mei_speaking "——你确定，这是讨厌？"
    xiang_speaking "……"
    mei_speaking "我想，一般人——是不会给讨厌的人穿自己的衣服，还怕她着凉的。"
    xiang_speaking "……"
    mei "向夏没说话，但我也不着急，只是默默地看着远处即将熄灭的火星。"
    xiang_speaking "你说得，有道理。"
    mei_speaking "嗯？"
    scene bg_black with dissolve
    mei "不知道自己在期待什么，我猛地转身，动作大到让身上轻轻披着的外套掉了出去。"
    mei "但此时此刻，我们都没在意这些。"
    show cg_x131 at cg_0 with dissolve
    mei "向夏也向前两步，站在我的面前。距离是如此近，以至于我能够看见她微微颤动的眼睛。"
    show cg_x132 at cg_0 with dissolve
    hide cg_x131
    xiang_speaking "我——"
    mei "四目相对的时候，她倏地低下头，又很快抬起来，直直地看着我。"
    show cg_x131 at cg_0 with dissolve
    hide cg_x132
    xiang_speaking "我想，也是。"
    show cg_x133 at cg_0 with dissolve
    hide cg_x131
    xiang_speaking "我应该也不会，想和讨厌的人接吻。"
    show cg_x134 at cg_0 with dissolve
    hide cg_x133
    mei_speaking "……"
    show cg_x135 at cg_0 with dissolve
    hide cg_x134
    mei_speaking "你终于承认了。"
    mei "我有些挑衅地冲她笑了笑，趁她还没说下一句话时凑上前去。"
    show cg_x136 at cg_0 with Fade(0.2,0.5,0.2)
    hide cg_x135
    mei "轻柔的触感转瞬即逝。很冷，一点也不温暖。但是……很舒服。"
    show cg_x137 at cg_0 with dissolve
    hide cg_x136
    mei "我又退后一步，眼睁睁看着向夏的脸瞬间涨红。"
    mei "糟糕，我自己好像也是……{p}哪怕只穿一件衣服，也觉得很热。"
    stop music fadeout 0.5
    pause(0.5)
    scene bg_sunset1 with vpunchm 
    $ renpy.music.play(music.xiang_he, channel="music", loop=True, fadein=0.5)
    xiang_speaking "你你——你！！"
    mei_speaking "我？"
    mei "我故作镇定，双手背在身后，正在不停地颤抖。"
    xiang_speaking "你要对我负责！"
    mei_speaking "……噗。"
    mei "我哑然失笑。然后，不知为何，在她又震惊又不知所措的表情中笑得更厉害了。"
    xiang_speaking "你笑什么啊！"
    mei_speaking "哈哈哈……对不起，但，哈，哈哈，哈哈哈……"
    xiang_speaking "喂！"
    mei_speaking "……哎呀，抱歉。\n我只是觉得，平时你看起来那么成熟，结果现在说的话跟中学生一样。"
    xiang_speaking "你才是中学生！我没读过书！"
    mei_speaking "好，好，知道了。我是我是。"
    xiang_speaking "你别——"
    mei_speaking "回去再说呗？在这里说话不好吧。"
    xiang_speaking "啊？那，走吧，快走！"
    mei_speaking "嗯，嗯，没问题。"
    mei "我看着她故作凶狠的表情——脸还是很红，所以毫无说服力——完全遏制不住脸上的笑容。"
    xiang_speaking "你得给我个说法——"
    mei_speaking "行，好，没问题，一定给。走吧，回去吧。"
    xiang_speaking "走就走！"
    $ persistent.pHE = 1
    window hide
    scene bg_white with Dissolve(4)
    pause(0.5)
    show screen endings_screen(endings,"xHE") with Pixellate(3,25)
    pause
    stop music fadeout 1.0

    return